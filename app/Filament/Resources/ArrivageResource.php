                ...
                Section::make('Détails des Produits Reçus')
                    ->schema([
                        Repeater::make('details_produits')
                            ->label('Produits')
                            ->schema([
                                // CORRECTION : Utilisation du "nom_complet" pour éviter toute confusion
                                Select::make('unite_de_vente_id')
                                    ->label('Unité de Vente')
                                    ->options(UniteDeVente::with('product')->get()->pluck('nom_complet', 'id'))
                ...
                Section::make('Résumé et Notes')
                    ->schema([
                        TextInput::make('montant_total_arrivage')
                            ...
                            ->prefix('FCFA')->numeric()->readOnly()->default(0)->dehydrated(true),
                        
                        TextInput::make('total_quantite')
                            ->label('Quantité Totale Reçue (unités)')
                            ->numeric()->readOnly()->default(0)->dehydrated(true),
