            ...
            @forelse($products as $product)
                <div class="product-card-info rounded-lg overflow-hidden group flex flex-col">
                    ...
                    <div class="p-6 flex flex-col flex-grow">
                        ...
                        @php
                            $selectedVariant = $product->uniteDeVentes->firstWhere('id', $selectedVariants[$product->id] ?? $product->uniteDeVentes->first()?->id);
                        @endphp
                        ...
                        <div class="mt-4">
                            <div class="flex items-center gap-2">
                                <input type="number" min="1" wire:model="quantities.{{ $product->id }}" class="w-20 form-input text-center bg-slate-800 border border-slate-700 rounded-md py-2 px-2 text-white">
                                <button wire:click="addToCart({{ $product->id }}, {{ $selectedVariant?->id }})" class="w-full bg-brand-red hover-bg-brand-red text-white font-bold text-sm uppercase tracking-wider py-3 px-3 rounded-md transition-colors">Ajouter</button>
                            </div>
                        </div>
                    </div>
                </div>
