<div>
    @if ($isSent)
        <div class="bg-green-900/50 border border-green-700 text-green-300 p-8 rounded-lg text-center">
            ...
            <p>Merci. Votre message a bien été enregistré. Notre équipe vous répondra dans les plus brefs délais.</p>
        </div>
    @else
        <form wire:submit.prevent="submit" class="bg-slate-900/50 p-8 rounded-lg border border-slate-800 space-y-6">
            <div class="grid md:grid-cols-2 gap-6">
                ...
                <div>
                    {{-- CORRECTION : Le champ email a été déplacé ici pour une meilleure disposition --}}
                    <label for="email" class="form-label">Votre adresse e-mail *</label>
                    <input type="email" id="email" wire:model="email" class="w-full bg-slate-800 border border-slate-700 rounded-md py-3 px-4 text-white placeholder-slate-500 focus:outline-none focus:border-brand-red focus:ring-1 focus:ring-brand-red">
            ...
            <div class="text-right">
                <button type="submit" class="bg-brand-red hover:bg-red-700 text-white font-bold tracking-widest uppercase transition duration-300 transform hover:scale-105 py-3 px-8 rounded-sm">
