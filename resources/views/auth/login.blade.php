        ...
        <div class="container mx-auto px-6 max-w-lg">
            <div class="bg-dark-card border border-border-dark rounded-lg">
                <div class="flex border-b border-border-dark">
                    <button @click="tab = 'partenaire'" :class="{ 'bg-primary text-white': tab === 'partenaire', 'text-slate-400': tab !== 'partenaire' }" class="flex-1 py-4 font-teko text-2xl uppercase tracking-wider transition-colors">Partenaire</button>
                    <button @click="tab = 'livreur'" :class="{ 'bg-primary text-white': tab === 'livreur', 'text-slate-400': tab !== 'livreur' }" class="flex-1 py-4 font-teko text-2xl uppercase tracking-wider transition-colors border-l border-border-dark">Livreur</button>
                </div>
                <div class="p-8">
                    <div x-show="tab === 'partenaire'" x-cloak>
                        ...
                        <form method="POST" action="{{ route('client.login.send_code') }}" class="space-y-6">
                            ...
                            <div class="pt-4"><button type="submit" class="w-full btn btn-primary py-3">Recevoir le code SMS</button></div>
                        </form>
                        <p class="text-center text-sm text-slate-400 mt-6">Pas encore partenaire ? <a href="{{ route('devenir-partenaire') }}" class="font-semibold text-primary hover:underline">Faites votre demande ici.</a></p>
                    </div>
                    <div x-show="tab === 'livreur'" x-cloak>
                        ...
                        <form method="POST" action="{{ route('livreur.login') }}" class="space-y-6">
                            ...
                            <div class="pt-4"><button type="submit" class="w-full btn btn-primary py-3">Se Connecter</button></div>
                        </form>
                    </div>
                </div>
            </div>
