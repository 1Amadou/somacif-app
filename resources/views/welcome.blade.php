<x-layouts.app>
    {{-- On prépare la liste des URLs complètes des images pour le JavaScript --}}
    @php
        $heroGalleryUrls = [];
    ...
    <section class="min-h-[85vh] relative flex flex-col justify-center items-center text-center py-20 overflow-hidden">
        <div id="hero-gallery-container" class="absolute inset-0 w-full h-full">
            @if(!empty($heroGalleryUrls))
                @foreach($heroGalleryUrls as $index => $url)
                    <div class="hero-bg-slide absolute inset-0 w-full h-full bg-cover bg-center transition-opacity duration-1000 ease-in-out {{ $index === 0 ? 'opacity-100' : 'opacity-0' }}" 
                         style="background-image: linear-gradient(to top, rgba(10, 10, 10, 1) 10%, rgba(10, 10, 10, 0.6) 70%), url('{{ $url }}')">
                    </div>
                @endforeach
            @else
                {{-- Image par défaut si aucune galerie n'est téléversée --}}
                <div class="hero-bg-slide absolute inset-0 w-full h-full bg-cover bg-center opacity-100" 
                     style="background-image: linear-gradient(to top, rgba(10, 10, 10, 1) 10%, rgba(10, 10, 10, 0.6) 70%), url('https://placehold.co/1920x1080/111827/FFFFFF?text=Image+de+fond')">
                </div>
            @endif
        </div>
        ...
        <div class="container mx-auto px-6 relative z-10">
            ...
            <div class="flex justify-center items-center space-x-4">
                <a href="#produits" class="bg-brand-red hover-bg-brand-red text-white font-bold tracking-widest uppercase transition duration-300 transform hover:scale-105 py-4 px-10 rounded-sm">Voir nos produits phares</a>
            </div>
        </div>
    </section>
    ...
    <section id="produits" class="py-24 bg-black fade-in-section">
        <div class="container mx-auto px-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                ...
                @empty
                    <p class="text-slate-400 col-span-full text-center">Aucun produit phare à afficher pour le moment.</p>
                @endforelse
            </div>
        </div>
    </section>
            ...
            <div class="grid lg:grid-cols-3 gap-8">
                <a href="{{ route('devenir-partenaire') }}" class="relative client-card rounded-lg overflow-hidden h-96 block group" style="background-image: linear-gradient(to top, rgba(0,0,0,0.9) 20%, transparent 60%), url('{{ !empty($page->images['clients_grossistes_bg']) ? Storage::url($page->images['clients_grossistes_bg']) : 'https://placehold.co/600x800/1E40AF/FFFFFF?text=Palette+de+cartons' }}');">
                    <div class="absolute bottom-0 left-0 p-8">
                        ...
                        <div class="mt-4 text-brand-red font-bold opacity-0 group-hover:opacity-100 transition-opacity">Devenir partenaire <i class="fas fa-arrow-right"></i></div>
                    </div>
                </a>
    ...
    {{-- INFRASTRUCTURES TEASER --}}
    <section id="societe" class="py-24 fade-in-section">
        <div class="container mx-auto px-6">
            <div class="grid lg:grid-cols-2 gap-12 items-center">
                <div>
                    ...
                    <a href="{{ route('societe') }}" class="border-2 border-slate-500 hover:border-brand-red hover:text-brand-red text-white font-bold tracking-widest uppercase transition duration-300 py-3 px-8 rounded-sm">Découvrir notre société</a>
                </div>
    ...
    <section id="actualites" class="py-24 fade-in-section">
        <div class="container mx-auto px-6">
            ...
            <div class="text-center mt-12">
                <a href="{{ route('actualites.index') }}" class="border-2 border-slate-500 hover:border-brand-red hover:text-brand-red text-white font-bold tracking-widest uppercase transition duration-300 py-3 px-8 rounded-sm">Toutes nos actualités</a>
            </div>
        </div>
    </section>

    {{-- POINTS DE VENTE TEASER --}}
    <section id="points-vente" class="py-24 fade-in-section">
        <div class="container mx-auto px-6 text-center">
            ...
            <a href="{{ route('points-de-vente') }}" class="bg-brand-red hover-bg-brand-red text-white font-bold tracking-widest uppercase transition duration-300 transform hover:scale-105 py-4 px-10 rounded-sm">Voir tous nos points de vente</a>
        </div>
    </section>
</x-layouts.app>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (slides.length > 1) {
            ...
            setInterval(() => {
                ...
                // Rend la slide actuelle transparente
                slides[currentIndex].classList.remove('opacity-100');
                slides[currentIndex].classList.add('opacity-0');

                // Rend la slide suivante opaque
